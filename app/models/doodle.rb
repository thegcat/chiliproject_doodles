class Doodle < ActiveRecord::Base
  ...
  acts_as_watchable
  acts_as_journalized :activity_find_options => {:include => :author},
                      :event_title => Proc.new {|o| "#{l(:label_doodle)} ##{o.journaled.id}: #{o.title}"},
                      :event_url => Proc.new {|o| {:controller => 'doodles', :action => 'show', :id => o.journaled}}
